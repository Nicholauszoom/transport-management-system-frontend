...
@Component({
 ...
 imports: [MenuComponent,
  ...
  templateUrl: './upload-taxi.component.html',
  styleUrl: './upload-taxi.component.css'
})
export class UploadTaxiComponent implements OnInit, OnDestroy {
  fileControl = new FormControl<File | null>(null);
  selectedFileName: string | null = null; 
  inProgress: boolean = false;
  private destroy$ = new Subject<void>();

  constructor(private taxiService: UploadTaxiService,
  ...
  ngOnInit(): void {
    this.taxiService.inProgress$.pipe(takeUntil(this.destroy$)).subscribe((progress) => {
  ...
  this.taxiService.uploadFile(file).subscribe({
    next: (response) => {
